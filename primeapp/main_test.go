...
func Test_checkNumbers(t *testing.T) {
	tests := []struct {
		...
		{name: "negative", input: "-1", expected: "Negative number are not prime, by definition!"},
		{name: "typed", input: "abc", expected: "Please enter a whole number!"},
...
func Test_prompt(t *testing.T) {
	...
	// perform out test
	if string(out) != "-> " {
		t.Errorf("prompt text not correct; excepted -> but got %s", string(out))
	}
}
...
func Test_isPrime(t *testing.T) {
	primeTests := []struct {
		...
		{"negative", -1, false, "Negative number are not prime, by definition!"},
